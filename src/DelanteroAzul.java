...
public class DelanteroAzul extends Entity {

	protected boolean collidePlayer = false;
	private int ticks = 0;
	private int origenX, origenY;
	public DelanteroAzul(int x, int y, ID id, Image sprite, Handler handler) {
					...
					if (collidePlayer == false)
						for (int i = 0 ; i < handler.object.size() ; i++) {
							...
							if (tempObject.getId() == ID.Ball) {
								if (tempObject.x > Cons.CANVAS_X/3)
		...
		for (int i = 0 ; i < handler.object.size() ; i++) {
			...
			if (tempObject.getId() == ID.Ball) {
				...
				if (bound.intersects(tempObject.bound)) {
					...
					ball.kickBall(Cons.CANVAS_X, Cons.CANVAS_Y/2 + (rng.nextInt(400) - 200));
				}
			}
